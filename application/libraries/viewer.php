...
class viewer
{
	...
	function view($body_view, $data=array(), $meta=false){
		...
		}elseif($this->mobile = $this->check_mobile() ){
			
			$this->uri_array = preg_split("[/]", $this->body);
			
			if(in_array($this->uri_array[0], $this->mobile_pages)){
	...
	function check_mobile(){
		...
		if($this->CI->session->userdata('browser_view') == 'mobile'){
			return true;
		}elseif($this->CI->session->userdata('browser_view') == 'desktop'){
			return false;
		 }else{
			if( $this->detect_mobile() ){
				$this->CI->session->set_userdata(array('browser_view' => 'mobile') );
				return true;
			}else{
				$this->CI->session->set_userdata(array('browser_view' => 'desktop') );
				return false;
			}
	...
	function detect_mobile($ipad=false,$iphone=true,$android=true,$opera=true,$blackberry=true,$palm=true,$windows=true,$mobileredirect=false,$desktopredirect=false){
		...
		switch(true){ // using a switch against the following statements which could return true is more efficient than the previous method of using if statements
			...
			case (stripos($user_agent, 'ipad')); 		// we find the words iphone or ipod in the user agent
			...
			case (stripos('ipod',$user_agent)||stripos('iphone',$user_agent)); // we find the words iphone or ipod in the user agent
			...
			case (stripos('android',$user_agent));  	// we find android in the user agent
			...
			case (stripos('opera mini',$user_agent)); 	// we find opera mini in the user agent
			...
			case (stripos('blackberry',$user_agent)); 	// we find blackberry in the user agent
